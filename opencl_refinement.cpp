...
void initOpenCL() {
    ...
    openCLDescriptors1 = cl::Buffer(openCLContext, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR, sizeof(float)*2*DESCRIPTORSIZE);
    openCLDescriptors2 = cl::Buffer(openCLContext, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR, sizeof(float)*2*DESCRIPTORSIZE);
...
void uploadMipMaps_openCL(const std::vector<cv::Mat> &mipMaps) {
    ...
    if (mipMaps.size() != mipMapWidths.size()) {
...
void sampleDescriptors_openCL(const int mipMap, std::vector<std::vector<Descriptor>>& destMips, const unsigned char* s, const float descriptorScale, const int width, const int height, const float mipScale) {
    ...
    openCLInt[2] = width;
    openCLInt[3] = height;
    openCLInt[4] = Descriptor::uint32count*32;
...
void refineKeyPoints_openCL(std::vector<KeyPoint> &destKeyPoints, std::vector<float>& destErrors, const int mipEnd, const int STEPCOUNT, const bool stepping, const float MIPSCALE, const float STEPSIZE, const float SCALEINVARIANCE, const float ROTATIONINVARIANCE, const std::function<void()> &parallel) {
    ...
    openCLInt[5] = Descriptor::uint32count * 32;
    ...
    for (int i = 0; i < 32; i++) {
        refineKeyPoints_cl.setArg(a, i < mipMapWidths.size() ? openCLMipMaps[i] : openCLMipMaps[0]); a++;
    }
    for (int i = 0; i < 32; i++) {
