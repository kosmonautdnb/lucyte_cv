...
cv::Mat output(const std::string& windowName, const cv::Mat& limage, const cv::Mat& rimage,
    ...
    for (int i = 0; i < keyPointsLeft.size(); i++) {
        if (errorsLeft[i] < MAXVARIANCEINPIXELS && lastFrameErrorsLeft[i] < MAXVARIANCEINPIXELS && errorsRight[i] < MAXVARIANCEINPIXELS && lastFrameErrorsRight[i] < MAXVARIANCEINPIXELS && fabsf(keyPointsLeft[i].y - keyPointsRight[i].y) < 3.0) {
            validKeyPoints++;
